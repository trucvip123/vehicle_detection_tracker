...
def test_plate_detection(image_path, use_gpu=None):
    ...
    plate_model = initialize_plate_detector(
        model_path="model/LP_detector.pt",
        device=use_gpu
    )
    ...
    if result["bbox"] is not None:
        ...
        if result["text"] is not None and result["text"] != "unknown":
            ...
            print(f"  - License plate text: Not recognized (OCR failed or not available)")
    else:
    ...
    if len(sys.argv) > 1:
        ...
        if screenshots_dir.exists():
            ...
            if vehicle_frames:
                ...
                print("❌ Error: No image specified and no vehicle_frame images found in screenshots/")
